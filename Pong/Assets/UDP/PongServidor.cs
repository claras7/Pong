...
public class PongServidor : MonoBehaviour
{
    ...
    private IPEndPoint remoteEndPoint;
    private bool clienteConectado = false;
    private bool jogoComecou = false; // Só inicia a bola quando cliente conectado
    private float velocidadeRaquete = 8f;
    ...
    private async Task ReceberMensagensAsync()
    {
        while (true)
        {
            ...
            // Detecta primeira conexão do cliente
            if (!clienteConectado)
            {
                clienteConectado = true;
                remoteEndPoint = result.RemoteEndPoint;
                ...
                // Reseta a bola para começar
                bool direcaoParaInimigo = Random.value > 0.5f;
                gm.bo.ResetarBola(direcaoParaInimigo);
            }

            // Movimenta raquete do cliente
            if (msg == "UP") MoverRaquete(raqueteCliente, 1);
            else if (msg == "DOWN") MoverRaquete(raqueteCliente, -1);
        }
    }
    ...
    private void Update()
    {
        if (!jogoComecou || gm.jogoAcabou) return;
    ...
    private void EnviarEstado()
    {
        if (!jogoComecou || remoteEndPoint == null) return;
